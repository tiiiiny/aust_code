...
for i in range(300):
    ...
    print(x_positions[1][i],y_positions[1][i])
...
# 准备保存到Excel的内容
headers = []  # 行标题，初始空值
headers = [f"龙头x (m)", f"龙头y (m)"]  # 龙头的x, y
for i in range(1, num_points-1):  # 龙身的各节
    headers += [f"第{i}节龙身x (m)", f"第{i}节龙身y (m)"]
headers += [f"龙尾x (m)", f"龙尾y (m)",f"龙尾（后）x (m)", f"龙尾（后）y (m)"]  # 龙尾的x, y
...
# # 遍历所有整数秒的时间步，提取各点的位置
# for i in range(len(x_positions)):  # 对每个点进行处理
#     row = []
...
# df = pd.DataFrame(data, index=headers, columns=time_columns) . wtm
df = pd.DataFrame(data,index=time_columns, columns=headers)

# 保存到Excel文件
df.T.to_excel("result1.xlsx", index=True)  # 转置并保存
