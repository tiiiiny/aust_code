...
def find_angle_decrease(a, arc_length, theta_start):
    """使用二分查找法找到每经过弧长1米的角度减少量"""
    low = 0
    high = 2 * np.pi
    # high = np.pi
    tolerance = 1e-6  # 精度要求
    ...
    while high - low > tolerance:
        mid = (low + high) / 2
        theta_end = theta_start -mid
        
        radius = arc_length_from_theta(theta_start, theta_end, a)
        current_arc_length = radius * mid
        
        if current_arc_length < arc_length:
            low = mid
        else:
            high = mid
