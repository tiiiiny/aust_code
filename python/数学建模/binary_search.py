...
'''
def find_delta_theta(a,theta_A,d):
    # 使用fsolve求解 delta_theta
    # a = p / (2 * np.pi)
    # theta_A = theta_A
    # d = d
    delta_theta_initial_guess = 0.1  # 初始猜测角度差
    # delta_theta = fsolve(equation, delta_theta_initial_guess,xtol=1e-12)[0]	
    #  # 调用fsolve，并传递额外的参数
    delta_theta = fsolve(equation, delta_theta_initial_guess, args=(a, theta_A, d), xtol=1e-12)[0]
    #fsolve 允许设置 xtol（解的精度），默认精度为 1e-8。可以进一步降低这个阈值，以提高求解精度。
    return delta_theta
'''
def find_delta_theta(a, theta_A, distance):
    delta_theta_initial_guess = 0.01  # 可以尝试更小的初始猜测值
